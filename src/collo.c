...
void make_f_matrix(int, int, double, double, void (*)(double, double*, double*, double*), double*, double*, double*, double*);
double difference(int, int, double*, double*);

int collo(int n, int nt, int m, double h, void (*F)(double, double*, double*, double*), double* x0, double* params, int s, double* c, double* res)
{
    ...
    for(i = 0; i <= nt; i++)
    {
        ...
        while(dif > DBL_EPSILON)
        {
            ...
            make_f_matrix(n, s, h, t_cur, F, params, y, c, f);
            for(j = 0; j < n; j++)
            {
                for(jj = 0; jj < s; jj++)
                {
...
void make_f_matrix(int n, int s, double h, double t0, void (*F)(double, double*, double*, double*), double* params, double* y, double* c, double* f)
{
    int i, j;
    double* ftmp;
    ...
    for(j = 0; j < s; j++)
    {
        F(t0 + h * c[j], y + n * j, params, ftmp + n * j);
        for(i = 0; i < n; i++)
        {
